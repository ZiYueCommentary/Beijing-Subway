...
* Classes in `ziyue.bjsubway.mod` **should not** be Javadoc'd with `@author` and `@see`.
...
* Classes for block shall always be Javadoc'd. `@see` should be linked to its block entity. If the superclass is located in `ziyue.bjsubway.mod.base`, add it to `@see` as well.
